...
class Solution {
    ...
    public int solution(int[] menu, int[] order, int k) {
        ...
        int[] time = new int[1100000];
        
        Deque<Integer> q = new ArrayDeque<>();
        /// 풀이 : queue의 size를 활용. 현재 시점을 기준으로 이전의 원소들을 queue에서 제거.
        for(int i=0; i<order.length; i++){
            while(!q.isEmpty()){
                if(q.peekFirst()<=k*i) q.poll();
                else break;
            }
            if(q.isEmpty()) q.add(k*i + menu[order[i]]);
            else q.add(q.peekLast()+menu[order[i]]);
